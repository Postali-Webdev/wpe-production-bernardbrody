...
class Knr_Keystone {
	...
	public static function generate_practice_area_link( string $office, string $practice_area ) : string {
		if ( 'akron' === $office || 'cleveland' === $office || 'columbus' === $office ) {
			return "/{$office}-office/practice-areas/{$practice_area}-accident-lawyers/";
		} else {
	...
	private function get_accident_data() : array {
		$url      = get_stylesheet_directory_uri() . '/json/crime-data/2017.json';
		$response = wp_safe_remote_get( $url, [ 'sslverify' => false ] );
		$body     = wp_remote_retrieve_body( $response );
		$data     = json_decode( $body, true );

		foreach ( $data['county_stats'] as &$county ) {
			$county['accidents']['per_age_group']['sum'] = array_sum( $county['accidents']['per_age_group'] );
		}

		$data['state_stats']['sum'] = array_sum( $data['state_stats'] );
	...
	public function enqueue_styles() {
		if ( is_page( 'interactive-ohio-traffic-data-map' ) || is_page( 'county' ) ) {
			...
			wp_enqueue_style( 'knr_keystone_aos', get_stylesheet_directory_uri() . '/css/aos.css' );
			wp_enqueue_style( 'knr_keystone_jsmaps_style', get_stylesheet_directory_uri() . '/lib/jsmap/jsmaps/jsmaps.css' );
			wp_enqueue_style( 'knr_keystone_style', get_stylesheet_directory_uri() . '/css/knr-keystone.css' );
		}
	}
	...
	public function enqueue_scripts() {
		if ( is_page( 'interactive-ohio-traffic-data-map' ) || is_page( 'county' ) ) {
			wp_enqueue_script( 'knr_keystone_jquery_ui', get_stylesheet_directory_uri() . '/js/jquery-ui.min.js', [ jquery ], true );
			wp_enqueue_script( 'knr_keystone_jquery_appear', get_stylesheet_directory_uri() . '/js/jquery.appear.js', [ jquery ], true );
			wp_enqueue_script( 'knr_keystone_jquery_animateNumber', get_stylesheet_directory_uri() . '/js/jquery.animateNumber.min.js', [ jquery ], true );
			...
			wp_enqueue_script( 'knr_keystone_aos', get_stylesheet_directory_uri() . '/js/aos.js', [ jquery ], true );
			wp_enqueue_script( 'knr_keystone_canvas', get_stylesheet_directory_uri() . '/js/canvasjs.min.js', [ jquery ], true );
			wp_enqueue_script( 'knr_keystone_highcharts', get_stylesheet_directory_uri() . '/js/highcharts.js', [ jquery ], true );
			wp_enqueue_script( 'knr_keystone_highcharts_3d', get_stylesheet_directory_uri() . '/js/highcharts-3d.js', [ jquery ], true );
			...
			if ( is_page( 'county' ) ) {
				...
				wp_enqueue_script( 'knr_keystone_county_script', get_stylesheet_directory_uri() . '/js/knr-keystone-county.js', [ jquery ], true );
			...
			if ( is_page( 'interactive-ohio-traffic-data-map' ) ) {
				wp_enqueue_script( 'knr_keystone_landing_script', get_stylesheet_directory_uri() . '/js/knr-keystone-landing.js', [ jquery ], true );
			}
		}
	}
}
